...
public class BossAiScriptTests
{
    ...
    [Fact]
    public void Beast_Ai_Uses_Mass_Sweep_When_Two_Enemies_In_Radius()
    {
        ...
        {
            var probe = Path.Combine(baseDir, "roles"); if (Directory.Exists(probe)) rolesDir = probe; baseDir = Directory.GetParent(baseDir)?.FullName ?? baseDir;
        }
        ...
        var after1 = (int)Convert.ToInt32(state.Units["P1"].Vars[Keys.Hp]);
        var after2 = (int)Convert.ToInt32(state.Units["P2"].Vars[Keys.Hp]);
        var newBossPos = (Coord)state.Units[bossId].Vars[Keys.Pos];
        int d1After = Math.Abs(newBossPos.X - ((Coord)state.Units["P1"].Vars[Keys.Pos]).X) + Math.Abs(newBossPos.Y - ((Coord)state.Units["P1"].Vars[Keys.Pos]).Y);
        int d2After = Math.Abs(newBossPos.X - ((Coord)state.Units["P2"].Vars[Keys.Pos]).X) + Math.Abs(newBossPos.Y - ((Coord)state.Units["P2"].Vars[Keys.Pos]).Y);
        Assert.True((after1 < before1 && after2 < before2) || (d1After + d2After < d1Before + d2Before));
    }
}
